...
import com.eazybytes.jensenstore.repository.RoleRepository;
...
public class AuthController {
    ...
    public ResponseEntity<ResponseDto> changePassword(@Valid @RequestBody ChangePasswordRequestDto changePasswordRequestDto) {
        ...
        boolean matches = passwordEncoder.matches(changePasswordRequestDto.getOldPassword(),customer.getPasswordHash());

        if (!matches) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResponseDto("400", "舊密碼不正確"));
        }
        CompromisedPasswordDecision check = compromisedPasswordChecker.check(changePasswordRequestDto.getNewPassword());
        if (check.isCompromised()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResponseDto("400", "密碼強度不足"));
        }
        customer.setPasswordHash(passwordEncoder.encode(changePasswordRequestDto.getNewPassword()));
        customerRepository.save(customer);
        return ResponseEntity.ok(new ResponseDto("200","密碼更新成功"));
    }
